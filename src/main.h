...
static const int LAST_POW_BLOCK = 1000; // at 60 second blocks will be about 30 days from launch 43200
...
static const int64_t MAX_MINT_PROOF_OF_STAKE = 0.06 * COIN;	// 10% annual interest
...
static const uint256 hashGenesisBlock("00000a58c31a5e4342d9a098b1f80bff0796915bd69e5e8f7a098eae8e3f96ad");
...
public:
    ...
    {
        // Loancoin: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CTxDB& txdb, uint64_t& nCoinAge) const;  // Loancoin: get transaction coin age
...
public:
    ...
    // Loancoin: block signature - signed by one of the coin base txout[N]'s owner
    ...
    // Loancoin: two types of block: proof-of-work or proof-of-stake
    ...
    // Loancoin: get max transaction timestamp
    ...
    bool AcceptBlock();
    bool GetCoinAge(uint64_t& nCoinAge) const; // Loancoin: calculate total coin age spent in block
...
public:
    ...
    unsigned int nFile;
    unsigned int nBlockPos;
    uint256 nChainTrust; // Loancoin: trust score of block chain
    int nHeight;

    int64_t nMint;
    int64_t nMoneySupply;

    unsigned int nFlags;  // Loancoin: block index flags
    enum  
    {
